...
- TimeSeries containers must expose `time_series` with:
...
class ModelVsObs2DStatisticsTimeSeries:
    ...
    def __init__(
        ...
        obs : object
            Container with attribute "time_series" exposing the observation dataset.
        obs_variable : str
        ...
        if 'name' in obs.time_series.attrs:
            self.obs_name = [obs.time_series.attrs['name']]
        else:
            self.obs_name = ['observations']

        # Isolate observations variable

        obs_data = obs.time_series.data_vars[obs_variable].values
        obs_data_units = obs.time_series[obs_variable].attrs['units']
        ...
        obs_time_index = np.where(obs.time_series['UTC'].values[:] == self.model_forecast_time)[0]
        ...
        # obs_time = obs.time_series['UTC'].values[obs_time_index]
        ...
        model_data = interpolation.model_2D_interpolate(
            ...
            model_lat_name,
            model_lon_name,
            obs.time_series.data_vars[obs_lat_name].values,
            obs.time_series.data_vars[obs_lon_name].values,
        )
        ...
        if 'long_name' in model_ds[model_variable].attrs:
            self.model_variable_long_name = TimeList([model_ds[model_variable].attrs['long_name']])
        else:
        ...
        if 'long_name' in obs.time_series[obs_variable].attrs:
            self.obs_variable_long_name = TimeList([obs.time_series[obs_variable].attrs['long_name']])
        else:
            self.obs_variable_long_name = TimeList([obs_variable])

        self.model_region_name = [obs.time_series.attrs['region']]
